...
__device__ float get_elemento (MATRIX mat, int i, int j){
    return mat.dados[i * mat.des + j];
}

__device__ void set_elemento (MATRIX mat, int i, int j, float val){
...
int main (int argc, char* argv[]){
    ...
    A.m = atoi (argv[1]);
    A.n = atoi (argv[2]);
    A.dados = (float*)malloc(A.m * A.n * sizeof(float));
    if (A.dados == NULL){
    ...
    B.m = A.n;
    B.n = atoi (argv[3]);
    B.dados = (float*)malloc(B.m * B.n * sizeof(float));
    if (B.dados == NULL){
    ...
    C.m = A.m;
    C.n = B.n;
    C.dados = (float*)malloc(C.m * C.n * sizeof(float));
    if (C.dados == NULL){
    ...
    if (gpu == 0){
        ...
        dA.m = A.m;
        dA.n = dA.des = A.n;
        e = cudaMalloc (&dA.dados, dA.m * dA.n * sizeof(float));
        if (e != cudaSuccess){
            printf("Erro no CUDA malloc A: %s\n",cudaGetErrorString(e));
            exit(1);
        }
        // Copiando A
        e = cudaMemcpy (dA.dados, A.dados, dA.m * dA.n * sizeof(float), cudaMemcpyHostToDevice);
        if (e != cudaSuccess){
        ...
        dB.m = B.m;
        dB.n = dB.des = B.n;
        e = cudaMalloc (&dB.dados, dB.m * dB.n * sizeof(float));
        if (e != cudaSuccess){
            printf("Erro no CUDA malloc B: %s\n",cudaGetErrorString(e));
            exit(1);
        }
        // Copiando A
        e = cudaMemcpy (dB.dados, B.dados, dB.m * dB.n * sizeof(float), cudaMemcpyHostToDevice);
        if (e != cudaSuccess){
        ...
        dC.m = C.m;
        dC.n = dC.des = C.n;
        e = cudaMalloc (&dC.dados, dC.m * dC.n * sizeof(float));
        if (e != cudaSuccess){
        ...
        e = cudaMemcpy (C.dados, dC.dados, dC.m * dC.n * sizeof(float), cudaMemcpyDeviceToHost);
        if (e != cudaSuccess){
